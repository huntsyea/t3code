...
## T27 + T31 — Atlas Rebrand (UI strings + packaging)

- **Branding flow:** `apps/desktop/src/app/DesktopEnvironment.ts` exports `APP_BASE_NAME` ("Atlas") which feeds `branding.{baseName, displayName}` via `resolveDesktopAppBranding`. The branding object is delivered to the web client via `window.desktopBridge.getAppBranding()` and consumed in `apps/web/src/branding.ts` as the `injectedDesktopAppBranding` fallback for `APP_BASE_NAME`. Updating both ends keeps Electron + browser-only modes consistent.
- **Legacy migration paths MUST stay verbatim:** `legacyUserDataDirName = isDevelopment ? "T3 Code (Dev)" : "T3 Code (Alpha)"` (DesktopEnvironment.ts:166) AND test fixtures referencing `T3 Code (Alpha)` paths (DesktopAppIdentity.test.ts:121, 141) and migration prompt strings in DesktopAtlasMigration.ts (T29 territory). These are intentional references to the OLD on-disk directory name and changing them would break user data migration.
- **Test fixtures with branding values must update in lockstep:** `DesktopAppIdentity.test.ts` asserts `calls.setName === ["T3 Code (Alpha)"]` based on `displayName` derived from `APP_BASE_NAME`. After the rename, must update to `"Atlas (Alpha)"`. Same for `appPath`/`resourcesPath` fixtures (test data should match the new bundle name `Atlas.app`) and `electronApp.name` mock returns ("T3 Code" → "Atlas").
- **`apps/desktop/package.json` `productName`:** the field `"productName": "T3 Code (Alpha)"` is read by electron-builder during packaging and by Electron at runtime. Changed to `"Atlas"` (no parenthetical) to match the new bundle name and the simplified Atlas branding.
- **Bundle ID stays `com.t3tools.t3code`:** plan T31 scope is "brew/winget/AUR identifiers" + oxlint plugin rename + README + CHANGELOG. Bundle IDs (`com.t3tools.t3code`, `T3CODE_COMMIT_HASH` env, etc.) are NOT in scope and changing them would break update-channel compatibility — leave alone.
- **Internal package names (`@t3tools/`, `t3tools` workspace prefix) must NOT change** per plan MUST NOT. Confirmed: only renamed `@t3tools/oxlint-plugin-t3code` → `@t3tools/oxlint-plugin-atlas` (the org stays `@t3tools`).
- **oxlint plugin rename — files that need updating:**
  - Directory: `git mv oxlint-plugin-t3code oxlint-plugin-atlas` (preserves git history)
  - `oxlint-plugin-atlas/package.json` `name` field
  - `oxlint-plugin-atlas/index.ts` plugin meta `name` ("t3code" → "atlas") — this changes the rule prefix exposed in `.oxlintrc.json`
  - `.oxlintrc.json` `jsPlugins` path AND rule key (`t3code/no-inline-schema-compile` → `atlas/no-inline-schema-compile`)
  - `package.json` `workspaces.packages[]` entry
  - `oxlint-plugin-atlas/test/utils.ts` plugin path constant + `jsPlugins[].name` + temp dir prefix
  - `oxlint-plugin-atlas/rules/*.test.ts` rule harness names
  - `scripts/release-smoke.ts` `workspaceFiles[]` entry
  - `bun.lock` regenerated automatically by `bun install`
- **Server-side "T3 Code" strings out of scope:** `apps/server/src/{cli/server.ts,bin.ts,startupAccess.ts,serverRuntimeStartup.ts,provider/Layers/*,checkpointing/Layers/CheckpointStore.ts,textGeneration/OpenCodeTextGeneration.ts,...}` contain ~25 user-visible "T3 Code" strings (CLI descriptions, provider status messages, git commit author identity, OpenCode title). T27 explicitly scopes UI strings to `apps/web` and `apps/desktop menus`. Server messages displayed in web UI are downstream and should be addressed in a follow-up task — premature inclusion risks side-effecting CLI/git author identity.
- **Test fixtures referencing fictional T3 Code content:** `apps/web/src/components/ChatView.browser.tsx:5853,5886` use a "T3 Code Adaptive Orchestration" plan title in test fixtures. These are user-visible (rendered in tests) so updated to "Atlas Adaptive Orchestration" to maintain branding consistency.
- **Pre-existing typecheck baseline (156 errors) unchanged by T27+T31:** all failing files (`server.test.ts`, `bin.test.ts`, `bin.ts`, `SafeVaultWrite.ts`, `ws.ts` line ~1151/1162, `031_ProjectionProjectsKind.test.ts`) are inherited from earlier tasks per prior learnings. T27+T31 introduces 0 new TS errors. `bun fmt` clean. `bun lint` clean (15 pre-existing warnings).
- **`bun install` regenerates bun.lock** after workspace package rename. Without it, lock entries `@t3tools/oxlint-plugin-t3code@workspace:oxlint-plugin-t3code` would point to a missing directory and break installs.
