...
export const createOxlintRuleHarness = (ruleName: string): RuleHarness => {
  ...
  const run: RuleHarness["run"] = Effect.fnUntraced(function* (source: string) {
    ...
    const fixtureDir = yield* fs.makeTempDirectoryScoped({ prefix: "atlas-oxlint-" });
    ...
    const pluginPath = path.join(repoRoot, "oxlint-plugin-atlas", "index.ts");

    yield* fs.writeFileString(
      configPath,
      yield* encodeOxlintConfig({
        jsPlugins: [{ name: "atlas", specifier: pluginPath }],
